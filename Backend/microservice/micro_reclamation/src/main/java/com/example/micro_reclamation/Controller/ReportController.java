...
public class ReportController {
    ...
    public ResponseEntity<byte[]> pdf(@RequestParam int mois, @RequestParam int annee) {
        byte[] pdfBytes = PdfGenerator.generate(reportService.generateMonthlyReport(mois, annee));
        return ResponseEntity.ok()
                .header("Content-Type", "application/pdf")
                .header("Content-Disposition",
                        "attachment; filename=\"rapport-" + mois + "-" + annee + ".pdf\"")
                .body(pdfBytes);
    }

    @GetMapping("/monthly/excel")
    public ResponseEntity<byte[]> excel(@RequestParam int mois, @RequestParam int annee) {
        byte[] xlsxBytes = ExcelGenerator.generate(reportService.generateMonthlyReport(mois, annee));
        return ResponseEntity.ok()
                .header("Content-Type",
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                .header("Content-Disposition",
                        "attachment; filename=\"rapport-" + mois + "-" + annee + ".xlsx\"")
                .body(xlsxBytes);
    }
